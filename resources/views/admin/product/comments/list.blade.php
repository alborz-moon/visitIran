    ...
    <div id="pro_search" class="flex gap30 margin20 flex-wrap hidden">
        <div class="flex gap10 center">
            ...
            <select id="confirmedFilter">
                ...
                <option {{ isset($confirmedFilter) && $confirmedFilter ? 'selected' : '' }} value="1">تایید شده</option>
                <option {{ isset($confirmedFilter) && !$confirmedFilter ? 'selected' : '' }} value="0">تایید نشده</option>
            </select>
        </div>
        ...
        <div class="flex gap10 center">
            ...
            <select id="orderBy">
                <option {{ isset($orderBy) && $orderBy == 'created_at' ? 'selected' : '' }} value="created_at">زمان ایجاد</option>
                <option {{ isset($orderBy) && $orderBy == 'rate' ? 'selected' : '' }} value="rate">امتیاز</option>
                <option {{ isset($orderBy) && $orderBy == 'confirmed_at' ? 'selected' : '' }} value="confirmed_at">زمان تایید</option>
            </select>
        </div>
        ...
        <tbody>
            ...
            @foreach($items as $item)
                <tr id="item_{{ $item['id'] }}">    
                    ...
                    <td>{{ $item['msg'] == null ? 'نظری ثبت نشده' : substr($item['msg'], 0, Str::length($item['msg']) > 30 ? 30 : Str::length($item['msg'])) . ' ...' }}</td>
